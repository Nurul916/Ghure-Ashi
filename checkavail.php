...
if(isset($_GET['tid'])){
    ...
    $sql="SELECT name,hire_rate,image,NID
            FROM tour_guide
            WHERE'$tid' LIKE id";

    $returnvalue=$conn->query($sql);
    $t=$returnvalue->fetchALL(PDO::FETCH_NUM);
    $tname="Tour Guide-".$t[0][0];
    $nid=$t[0][3];


    $sql="SELECT * 
          FROM booking 
          WHERE ((end_date BETWEEN '$cin' AND '$cout') OR ('$cin' BETWEEN arrival_date AND end_date) OR (arrival_date BETWEEN '$cin' AND '$cout')) AND '$nid' LIKE booking_details ";
   ...
   <?php
    ...
    {?>
        ...
        <div class="text-center" style="margin: 240px;">
            <h3 class="featurette-heading">Sorry Tour Guide "<?php echo $t[0][0] ?>" Is Not Available </h3>
        </div>
    </div>
    <?php
    }
    ...
    <div class="container my-5">
        <div class="row featurette d-flex justify-content-center align-items-center">
            <div class="col-md-7 order-md-1">
                ...
                <h4><span class="text-muted">Status: Avilabe</span></h4>
                <p class="lead">Total: <?php $total=$d->format('%d')*(int)$t[0][1]; echo $total; ?> BDT</i></p>
                <form class="form-inline">
                    <div class="form-group mb-2">
                    </div>
                    <?php if(isset($_SESSION['email'])){?>
                    <a href="checkout.php?cin=<?php echo $cin ?>&cout=<?php echo $cout?> &hid=<?php echo $tid?> &rnmbr=<?php echo $nid?>&hname=<?php echo $tname?>&price=<?php echo $total?>"><button type="button" class="btn btn-primary mb-2 ">Book Now</button></a>
            ...
            <div class="col-md-5 order-md-2">
                <?php echo '<img class="bd-placeholder-img bd-placeholder-img-lg featurette-image img-fluid mx-auto" width="500"
                    height="500" src="data:image/jpeg;base64,'.base64_encode( $t[0][2] ).'"alt=""/>';?>
            </div>
        </div>
    </div>
