...
$con = mysqli_connect("localhost", "root","", "ghureashi");
    ...
    {
        $query= "SELECT * FROM `nearby_touristspot`";
        $queryfire = mysqli_query($con,$query);
        $num= mysqli_num_rows($queryfire);   
    }
    ...
    if(isset($_POST['id']))
    {
        ...
        $query1= "DELETE FROM `nearby_touristspot` WHERE id LIKE '$r_id'";
...
<body>
    ...
    <div class="container">
        <form class="form-horizontal my-3" method="post" action=" ">
                            ...
                            if(isset($_SESSION['Aemail'])){
                            ?>
                            <a href="write_touristspot.php" class="btn btn-primary mb-5 mt-3 float-right" aria-label="Search" name="wb" id="wb"><strong>Add Tourist Spot</strong></a>
                             <?php } ?>
                            </div>
...
<br>
    ...
    <div class="container my-5">
        <div class="text-center" style="margin: 150px;">
            <h2 class="featurette-heading">Sorry No Tourist Spot Found. </h2>
        </div>
    </div>
    <?php
    }
    ...
    <div class="container my-5">
        <div class="row featurette d-flex justify-content-center align-items-center">
            <div class="col-md-7 order-md-1">
                <form action="add_remove_touristspot.php" class="mb-3" method="post">
                 <h2 class="featurette-heading">ID: <?php echo $details['id'] ?> </h2>
                <h2><span class="text-muted">Name: <?php echo $details['name'] ?></span></h2>
                <p class="lead">Area: <?php echo $details['area'] ?></p>
                <p class="lead">Location: <?php echo $details['location'] ?></p>
                <p class="lead">Latitude: <?php echo $details['latitude'] ?></p>
                <p class="lead">Longitude: <?php echo $details['longitude'] ?></p>
